...
from pylab import figure,plot,show,xlim,ylim,title,legend,xlabel,ylabel,savefig,xticks,yticks
...
title('Temperature=%.1f, Protein length=%d'%(T,N), fontsize=25)
plot(energy_array, color='k')
xlabel('Monte Carlo step', fontsize=20)
ylabel('Energy', fontsize=20)
xticks(fontsize=15)
yticks(fontsize=15)
savefig('energy_vs_step_T%d_N%d_n%d.png'%(10*T,N,n), bbox_inches='tight')
show()

figure(figsize=(12,10))
plot(monomer_coords[:,0],monomer_coords[:,1],'-k') # plot bonds
title('Temperature=%.1f, Energy = %.1f'%(T,energy), fontsize=25)
# plot monomers
for i in range(N):
...
xticks(fontsize=15)
yticks(fontsize=15)
savefig('final_protein_T%d_N%d_n%d.png'%(10*T,N,n), bbox_inches='tight')
show()
